...
const RESPONSES = {
  Stressed: [
    ...
    { text: "You have logged stress recently. Even 5 deep breaths can help.", action: { label: 'Open Breathe', path: '/breathe' } },
  ],
  Worried: [
    { text: "Feeling worried is okay. Try 4-7-8 breathing to calm your nervous system.", action: { label: 'Try 4-7-8', path: '/breathe' } },
    { text: "Write down what is worrying you. Journaling helps release mental weight.", action: { label: 'Open Journal', path: '/journal' } },
  ],
  Tired: [
  ...
  Low: [
    { text: "Feeling low happens. Read today's affirmations — they are written for moments like this.", action: { label: 'View Affirmations', path: '/library' } },
  ...
  Happy: [
    { text: "You are in a great mood! Channel this energy into a focus session.", action: { label: 'Start Focus', path: '/timer' } },
    { text: "Love seeing you happy! Keep your streak going — journal today too.", action: { label: 'Add Entry', path: '/journal' } },
  ],
  Peaceful: [
    { text: "You are feeling peaceful. This is a perfect time to focus deeply.", action: { label: 'Start Focus', path: '/timer' } },
    { text: "Peaceful energy is rare — use it! A focus session now will feel effortless.", action: { label: 'Start Timer', path: '/timer' } },
  ],
  Energised: [
    { text: "You are energised! Do not waste it — start a Pomodoro session right now.", action: { label: 'Start Focus', path: '/timer' } },
...
export default function MindfulnessAssistant() {
  ...
  const sendGreeting = () => {
    ...
    setMessages([{
      id: Date.now(),
      text: timeGreet + '! I am your mindfulness assistant. Let me check how you are doing...',
      type: 'assistant',
    }])
    setTyping(true)
    setTimeout(() => {
  ...
  const handleUserReply = (reply) => {
    ...
    setTimeout(() => {
      ...
      if (keywords.includes('stress') || keywords.includes('anxious') || keywords.includes('worried')) {
        ...
        response = { text: "A 25-min Pomodoro session with study music is the best way to focus.", action: { label: 'Start Focus', path: '/timer' } }
      } else if (keywords.includes('breath') || keywords.includes('calm')) {
        response = { text: "Box breathing for 2 minutes works really well to calm down.", action: { label: 'Open Breathe', path: '/breathe' } }
      } else {
