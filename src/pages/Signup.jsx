  ...
  return (
    <div style={{ minHeight: 'calc(100vh - 72px)', paddingTop: 72, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '100px 24px 40px', position: 'relative', zIndex: 1 }}>
      <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}
        ...
        <p style={{ color: '#8c7fa0', fontSize: '0.88rem', marginBottom: 36 }}>Start your journey to calm</p>
        ...
        <form onSubmit={handleSubmit}>
          <div style={{ marginBottom: 18 }}>
            ...
            <input className="seren-input" type="text" placeholder="Your name"
              value={name} onChange={e => setName(e.target.value)} />
            {errors.name && <p style={{ fontSize: '0.8rem', color: '#e07090', marginTop: 5 }}>{errors.name}</p>}
          </div>
          ...
          <div style={{ marginBottom: 28 }}>
            ...
            <input className="seren-input" type="password" placeholder="At least 6 characters"
        ...
        <p style={{ textAlign: 'center', marginTop: 22, fontSize: '0.85rem', color: '#8c7fa0' }}>
          Already have an account? <Link to="/login" style={{ color: '#a78bca', textDecoration: 'none', fontWeight: 500 }}>Sign in</Link>
        </p>
      </motion.div>
    </div>
  )
